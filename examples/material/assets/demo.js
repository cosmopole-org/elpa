...
//
// Components are plain functions `(props, update) => widget`, React-style, placed
// in the tree with `Component(fn, props)` so the runtime can re-run **only that
// component** when its `update` is called. A handler mutates state and calls
// `update()` (the Flutter `setState` pattern); the SDK repaints just that
// component and reassembles the frame from the cached output of everything else.
// State that affects several widgets is owned higher up (the radios are
// self-contained, so they live in their own component and update in isolation).
...
// --- custom widgets: plain functions returning a widget tree ------------------
// A caption stacked above a control. No state, so `update` is unused.
function Tile(props, update) {
...
function RadioRow(props, update) {
    let names = ["A", "B", "C"];
    let kids = [];
    for (let i = 0; i < 3; i++) {
        let idx = i;
        push(kids, Component(Tile, { label: names[idx], child: Radio({
            id: concat("r", str(idx)),
    ...
    return Scaffold({
        ...
        body: Card({ child: Column({ gap: 3.0, children: [
            ...
            Row({ gap: 7.0, children: [
                Component(Tile, { label: "WI-FI", child: Switch({ id: "wifi", value: swOn, onTap: () => { swOn = 1.0 - swOn; update(); } }) }),
                Component(Tile, { label: "AGREE", child: Checkbox({ id: "agree", value: ck, onTap: () => { ck = 1.0 - ck; update(); } }) }),
            ] }),
            Component(Tile, { label: "VOLUME", child: Slider({ value: vol, onChanged: (v) => { vol = v; update(); } }) }),
            Component(Tile, { label: "FILTER", child: Chip({ id: "filter", label: "FILTER", value: chip,
                onTap: () => { chip = 1.0 - chip; update(); } }) }),
            Component(RadioRow, {}),
            Divider({}),
            Component(Tile, { label: "TASKS", child: Progress({ id: "tasks", value: (swOn + ck + chip) / 3.0 }) }),
        ] }) }),
    });
}
