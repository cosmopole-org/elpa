...
class FontEngine {
    ...
    atlasId() { return concat("elpa.web.atlas.", str(this.fontVer)); }
    atlasTexRes() {
        ...
        return [
            ...
            { kind: "sampler", id: "elpa.web.samp", mag_filter: "linear", min_filter: "linear", mipmap_filter: "linear" },
        ];
    }
...
class MediaEngine {
    ...
    imgTexId(handle, w, h, ver) { return concat(concat(concat(concat(concat(concat("elpa.web.img.tex.", str(handle)), "."), str(w)), concat("x", str(h))), ".v"), str(ver)); }
    ...
    imgDrawCmds(plan, vbuf, tag, res, uploads, declared) {
        ...
        for (let i = 0; i < len(plan.draws); i++) {
            let d = plan.draws[i];
            if (d.sdf > 0.5) {
                push(pcmds, { cmd: "setBindGroup", index: 0, bind_group: "elpa.web.gb" });
                push(pcmds, { cmd: "setPipeline", pipeline: "elpa.web.pipe" });
                ...
                let uid = concat(concat("elpa.web.img.u.", tag), str(i)); let bid = concat(concat("elpa.web.img.bg.", tag), str(i));
                ...
                push(pcmds, { cmd: "setPipeline", pipeline: "elpa.web.img.pipe" });
    ...
    imgPipelineResources() {
        return [
            { kind: "shader", id: "elpa.web.img.shader", wgsl: IMG_WGSL },
            { kind: "bindGroupLayout", id: "elpa.web.img.bgl",
              entries: [
            ...
            { kind: "pipelineLayout", id: "elpa.web.img.layout", bind_group_layouts: ["elpa.web.img.bgl"] },
            { kind: "renderPipeline", id: "elpa.web.img.pipe", layout: "elpa.web.img.layout",
              vertex: { module: "elpa.web.img.shader", entry_point: "vs", buffers: [] },
              fragment: { module: "elpa.web.img.shader", entry_point: "fs", targets: [{
                  format: SURFACE_FMT,
            ...
            { kind: "sampler", id: "elpa.web.img.samp", mag_filter: "linear", min_filter: "linear", mipmap_filter: "linear" },
        ];
    }
    addImgPipeline(res) {
